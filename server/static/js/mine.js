let pres = document.getElementById('sam').getContext('2d');
let allCandChart = new Chart(pres,{
  type:'bar',//bar,horizontalBar,pie,line,doughnut,radar,polarArea
  data:{
    labels:[],
    datasets:[{
      label:'Number of Votes per Candidate',
      data:[],
      backgroundColor:[
    'rgba(255, 99, 132, 0.2)',
    'rgba(54, 162, 235, 0.2)',
  ...
  options:{
    ...
    title:{
      display:true,
      text:'Candidate By Total vote count',
      fontSize:25
    },
  },
  legend:{
...
var cans = [];
/*console.log(cans[1].name);*/
var url = "http://localhost:2000/agp";
var reqData = new XMLHttpRequest();
reqData.open('GET',url);
//console.log(JSON.parse(reqData.responseText));
reqData.onload = function(){
  console.log(this.responseText); //CLSDHtwwwDqLj
  var can = this.responseText;
  var cans = JSON.parse(can);
  //console.log(cans[1].votes);
  for(var i = 0;i < cans.length; i++){
    allCandChart.data.labels.push(cans[i].name);
    console.log(i.votes);
    allCandChart.data.datasets[0].data.push(cans[i].votes);
    allCandChart.update();
  }
};
reqData.send();
